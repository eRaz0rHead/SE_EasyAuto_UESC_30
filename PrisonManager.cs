...
class PrisonManager: EasyAPI {
  ...
  Dictionary < int, PrisonCell > prisons;
  List < int > rotorLinks = [3, 2, 1];
  void Init() {
    ...
    void FindCell(string arg) {
     System.Text.RegularExpressions.Match m =
      (new System.Text.RegularExpressions.Regex(“([1 - 5])([A - D]”)).Match(arg);
       if (!m.Success) {
        Echo(‘No Prison called‘ + arg + ‘found‘);
        return;
       }
       var rotor = m.Groups[1].Value;
       var chamber = m.Groups[2].Value; PrisonCell cell = prisons[rotor]; cell.requestedChamber = chamber;
       return cell;
      }
    }
    ...
    class PrisonCell {
     ...
     PrisonCell(EasyBlock rotor) {
      this.rotor = rotor;
      System.Text.RegularExpressions.Match m =
       (new System.Text.RegularExpressions.Regex(CellPattern).Match(rotor.Name());
        if (m.Success) {
